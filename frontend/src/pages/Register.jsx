...
import { UserPlus, MessageSquare, AlertCircle, ArrowLeft } from "lucide-react";
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass border border-slate-800 rounded-2xl p-8 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-slate-400 text-sm mt-1 font-sans">Get started with ChatSphere AI</p>
        </div>

        {error && (
          <div className="flex items-center gap-3 bg-red-950/40 border border-red-800/60 text-red-200 p-4 rounded-xl text-sm mb-5 animate-shake">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              required
              className="w-full bg-slate-900/60 border border-slate-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-brand-500 transition-all focus:ring-1 focus:ring-brand-500/30"
              placeholder="pavan"
              value={username}
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-slate-900 hover:bg-slate-850 border border-slate-800 text-slate-200 py-2.5 rounded-xl flex items-center justify-center gap-3 transition-all hover:scale-[1.01] active:scale-[0.99]"
        >
        ...
        <p className="text-center text-slate-400 text-sm mt-6">
          <Link to="/login" className="text-slate-400 hover:text-white inline-flex items-center gap-1.5 hover:underline">
